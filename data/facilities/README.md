...
```bash
# 1) 공공데이터포털·열린데이터광장에서 CSV 를 받는다 (로그인 필요)
# 2) UTF-8 로 저장한 뒤 변환한다
go run ./cmd/importfacilities \
  -in ~/Downloads/서울시_지역아동센터.csv \
  -out data/facilities/seoul-child-center.json \
  -type CHILD_CENTER -sido 서울특별시 -prefix seoul
```

`-type` 을 비우면 `시설종류명` 컬럼에서 유추한다. 못 알아보면 `OTHER` 다 —
틀린 분류로 넣는 것보다 낫다.

임포터가 하는 일

- 헤더 이름을 여러 후보로 찾는다 (`시설명` / `기관명` / `센터명` …).
  **못 찾은 컬럼은 끝에 알려준다** — 조용히 빈 값으로 채우지 않는다
- 주소에서 시도·시군구를 읽어 `coverage` 를 `SIGUNGU` 로 채운다
- 홈페이지에 `https://` 를 붙인다
- ★ **아래 행은 건너뛰고 이유를 남긴다**
  - 전화·홈페이지가 모두 없는 행 (연락할 수 없는 시설)
  - 주소에서 시군구를 읽지 못한 행 (관할을 정할 수 없다)
  - 시설명이 빈 행

★ **임포터가 만들 수 없어서 사람이 채워야 하는 것**

| 항목 | 왜 사람이 채워야 하나 |
|---|---|
| `summary` | "이 시설이 나에게 뭘 해주나" 는 원본 데이터에 없다 |
| `services` | 제공 서비스. 시설 홈페이지·전화 확인이 필요하다 |
| `fee` | 이용료 |
| `documents` | 방문 시 준비물 |
| `eligibility` | 이용 대상 조건. 비우면 "관할 주민 누구나" |
| `contact.hours` | 운영시간. 원본에 없거나 자유 문자열이라 신뢰할 수 없다 |

**이게 새 병목이다.** 목록은 자동으로 들어오지만, 이용자에게 실제로
도움이 되는 정보는 사람이 확인해서 채워야 한다.
...
- `seoul-mental-health.json` — 서울 25개 자치구 정신건강복지센터 (확인 완료)
  - 출처: 블루터치(서울시정신건강복지센터) <https://blutouch.net/facility/center>
  - 자치구마다 하나씩 있어 **관할 판정을 서울 전역에서 확인할 수 있다**
  - ★ 운영시간은 센터마다 달라 비워 두었다. 화면에서 "전화로 확인" 을 안내할 것
